...
from .optimizers.drago import DRAGO, DRAGOForClasses
...
class ImportanceLoss(torch.nn.Module):
    def __init__(self, loss_fn , tau = 1, C = 2,  warmup_steps = 100):
        super().__init__()
        self.tau = tau
        self.C = C
    ...
    def clip_importance_loss_fn(self,x,y):
        return self.base_loss(x,y , reduction='none').mul(self.tau).clip(-self.C,self.C).exp().mean().sub(1).divide(self.tau)
...
class ScheduleLoss(torch.nn.Module):
    def __init__(self, base_loss_fn, new_loss_fn, hyperparam, gamma, warmup):
        super().__init__()
        self.base_loss = base_loss_fn
        self.new_loss = new_loss_fn

        self.hyperparam = hyperparam    # main hyperparameter of new loss
        self.gamma = gamma              # scheduling speed

        self.warmup = warmup
        self.__scaled_hyperparam = hyperparam
        self.__current_iter = 0

    def forward(self, x, y, type_scheduling='hyperparam gamma scaling'):
        self.__current_iter += 1
                    
        if type_scheduling == 'hyperparam gamma scaling':
            if self.__current_iter < self.warmup:
                self.__scaled_hyperparam = self.__scaled_hyperparam * self.gamma 
            return self.new_loss(x,y,self.__scaled_hyperparam)            
        elif type_scheduling == 'alpha iteration decreasing':
            if self.__current_iter < self.warmup:
                alpha = 1.0 / self.__current_iter
            else:
                alpha = 0                    
            return alpha * self.base_loss(x,y) + (1 - alpha) * self.new_loss(x,y,self.hyperparam) 
                    
        else:
            raise NotImplementedError(f'Not type scheduling: {type_scheduling}')
...
def make_optimizer(
    ...
    if type == 'Drago':
        return DRAGO(**kwargs)
    if type == 'DragoForClasses':
        return DRAGOForClasses(**kwargs)
    if type == 'ALSO':
        return ALSO(**kwargs)
...
def zero_grad_forward_backward(
    ...
    weights: Tensor | None,
    scale: float | None,
    grad_scaler: None | torch.cuda.amp.GradScaler = None, 
        ...
        try:
            if batch_size <= chunk_size:
                ...
                if weights is not None and scale is not None:
                    losses = scale * step_fn(batch_idx)
                    loss = (weights * losses).sum()
                else:
                    loss = step_fn(batch_idx).mean()
                backward(loss)
            else:
                ...
                loss = None
                losses = None
                indexes = torch.arange(len(batch_idx))
                for chunk_idx_ in indexes.split(split_size=chunk_size):
                    chunk_idx = batch_idx[chunk_idx_]
                    chunk_losses = None
                    if weights is not None and scale is not None:
                        chunk_losses = scale * step_fn(chunk_idx)
                        if weights is None and compute_weights_fn is not None:
                            weights = compute_weights_fn(losses)
                            chunk_loss = (weights * chunk_losses).sum()
                            weights = None
                        else:
                            chunk_losses = scale * chunk_losses
                            chunk_loss = (weights[chunk_idx_] * chunk_losses).sum()
                    else:
                        chunk_loss = step_fn(chunk_idx).mean()
                    ...
                    if weights is not None and scale is not None:
                        if losses is None:
                            losses = chunk_losses.detach()
                        else:
                            losses = torch.cat((losses, chunk_losses.detach()))
    ...
    return cast(Tensor, loss), losses, chunk_size


def zero_grad_forward_backward_orig(
    optimizer: torch.optim.Optimizer,
