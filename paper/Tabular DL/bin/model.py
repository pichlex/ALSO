...
print("NUM_EMBEDDINGS VERSION", rtdl_num_embeddings.__version__)
...
import lib.deep
import lib.env
from lib.optimizers.deshift import make_spectral_risk_measure, make_extremile_spectrum
from lib.optimizers.large_scale_dro import RobustLoss
...
def main(
    ...
    *,
    ...
    static_weights = None
    if 'loss' in config['optimizer']:
        if config['optimizer']['loss'] == "importance":
            tau = config['optimizer']['loss_tau']
            def loss_fn(y_pred: Tensor, y_true: Tensor) -> Tensor:
                return _loss_fn(y_pred.flatten(0, 1), y_true.repeat_interleave(y_pred.shape[1]), reduction='none').mul(tau).exp().mean().sub(1).divide(tau)

        elif config['optimizer']['loss'] == 'dro':
            def loss_fn(y_pred: Tensor, y_true: Tensor) -> Tensor:
                return _loss_fn(y_pred.flatten(0, 1), y_true.repeat_interleave(y_pred.shape[1]), reduction='none')
            spectrum = make_extremile_spectrum(batch_size, n_draws=config['optimizer']['loss_ndraws'])
            compute_weights_fn = make_spectral_risk_measure(spectrum, penalty=config['optimizer']['loss_penalty'] , shift_cost=config['optimizer']['loss_shift_cost'])
        
        elif config['optimizer']['loss'] == 'dro_ls':
            def base_loss_fn(y_pred: Tensor, y_true: Tensor) -> Tensor:
                return _loss_fn(y_pred.flatten(0, 1), y_true.repeat_interleave(y_pred.shape[1]), reduction='none')
            loss_fn = RobustLoss(
                        base_loss_fn=base_loss_fn,
                        size=config['optimizer']['loss_size'],
                        reg=config['optimizer']['loss_reg'],
                        geometry=config['optimizer']['loss_geometry']
                    )
        
        elif config['optimizer']['loss'] == 'static_weights':
            def loss_fn(y_pred: Tensor, y_true: Tensor) -> Tensor:
                return _loss_fn(y_pred.flatten(0, 1), y_true.repeat_interleave(y_pred.shape[1]), reduction='none')
            targets = np.array(dataset.data['y']['train'].cpu())
            if dataset.task.is_regression:
                def get_quant(x):
                    " for each value in x, return which quantile it corresponds to "
                    return np.array([np.count_nonzero(x<x_i)/(len(x)-1) for x_i in x])
                static_weights = np.abs(get_quant(targets) - 0.5) + 1
            else:
                class_weights = compute_class_weight(class_weight="balanced", classes=np.unique(targets), y=targets)
                class_weights /= class_weights.sum()
                class_weights = class_weights * len(np.unique(targets))
                static_weights = class_weights[targets]
            static_weights = torch.from_numpy(static_weights).to(dtype=next(model.parameters()).dtype, device=device)


        loss_keys = []
        for key in config['optimizer']:
            if 'loss' in key:
    ...
    if config['optimizer']['type'] not in ['Drago']:
        if 'beta1' not in config['optimizer']:
            config['optimizer']['beta1'] = 0.9
        if 'beta2' not in config['optimizer']:
            config['optimizer']['beta2'] = 0.999
        config['optimizer']['betas'] = (config['optimizer']['beta1'], config['optimizer']['beta2'])
        del config['optimizer']['beta1']
        del config['optimizer']['beta2']
    if config['optimizer']['type'] in ['AdamW', 'Adam']:
        optimizer = lib.deep.make_optimizer(
            **config['optimizer'], params=lib.deep.make_parameter_groups(model)
        )
    elif config['optimizer']['type'] in ['ALSO', 'Drago']:
        if  'init_statis_weights' in config['optimizer']:
            del config['optimizer']['init_statis_weights']
            targets = np.array(dataset.data['y']['train'].cpu())
            if dataset.task.is_regression:
                def get_quant(x):
                    " for each value in x, return which quantile it corresponds to "
                    return np.array([np.count_nonzero(x<x_i)/(len(x)-1) for x_i in x])
                pi_reg = np.abs(get_quant(targets) - 0.5) + 1
                pi_reg = pi_reg / pi_reg.sum()
            else:
                pi_reg = compute_class_weight(class_weight="balanced", classes=np.unique(targets), y=targets)
                pi_reg /= pi_reg.sum()
                pi_reg = pi_reg * len(np.unique(targets))
                pi_reg = pi_reg[targets]
            pi_reg = torch.tensor(pi_reg, requires_grad=False).to(dtype=next(model.parameters()).dtype)
        else:
            pi_reg = None
        optimizer = lib.deep.make_optimizer(
            **config['optimizer'], 
            n_groups=len(dataset.data['y']['train']), 
            batch_size=config['batch_size'], 
        ...
        raise NotImplemented(f"There is not optimizer {config['optimizer']['type']}")
    ...
    def save_checkpoint() -> None:
        try:
            with open(Path(output) / 'optimizer_logs.pickle', 'wb') as handle:
                pickle.dump(optimizer.logs, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    while config['n_epochs'] == -1 or step // epoch_size < config['n_epochs']:
        ...
        for batch_idx in tqdm(
            ...
            def closure(weights=None, scale=None, compute_weights_fn=None, return_chunk_size=False):
                loss, losses, new_chunk_size = lib.deep.zero_grad_forward_backward(
                    optimizer=optimizer,
                    step_fn=lambda idx: loss_fn(apply_model('train', idx), Y_train[idx]),
                    batch_idx=batch_idx,
                ...
                if weights is not None and scale is not None and not return_chunk_size:
                    return losses, loss
                return loss, new_chunk_size
            closure.device = device

            if static_weights is None:
                loss, new_chunk_size = closure(compute_weights_fn=compute_weights_fn, return_chunk_size=True)
            else:
                loss, new_chunk_size = closure(
                                                weights=static_weights[batch_idx],
                                                scale=1/batch_size, 
                                                compute_weights_fn=compute_weights_fn,
            ...
            if config['optimizer']['type'] in ['AdamW', 'Adam']:
                ...
                if grad_scaler is None:
                    optimizer.step(closure, groups_indexes=batch_idx)
                else:
                    grad_scaler.step(optimizer, closure=closure, groups_indexes=batch_idx)
                    grad_scaler.update()
