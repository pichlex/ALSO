...
DEFAULT_REFERENCE_CONFIG = PROJECT_ROOT / "configs-nips" / "test_config.json"
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "configs-nips" / "sgd" / "cifar10"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate configs for NIPS runs "
            "(default profile: configs-nips/test_config.json)."
        )
    )
    parser.add_argument(
        "--reference-config",
        ...
        help="Reference JSON to inherit the base hyperparameters from.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    parser.add_argument("--scheduler-T-max", dest="scheduler_T_max", type=int, default=None)
    parser.add_argument("--scheduler-eta-min", dest="scheduler_eta_min", type=float, default=None)
    parser.add_argument("--augment", action=argparse.BooleanOptionalAction, default=None)
    parser.add_argument("--adaptive-min", dest="adaptive_batch_min", type=int, default=None)
    parser.add_argument("--adaptive-max", dest="adaptive_batch_max", type=int, default=None)
    parser.add_argument("--adaptive-beta", dest="adaptive_batch_beta", type=float, default=None)
    parser.add_argument("--report-to", dest="report_to", default=None)
    parser.add_argument(
        "--use-old-tune-params",
        dest="use_old_tune_params",
        action=argparse.BooleanOptionalAction,
        default=None,
    )
    parser.add_argument("--tune-runs", dest="tune_runs", type=int, default=None)
    parser.add_argument("--n-epoches-tune", dest="n_epoches_tune", type=int, default=None)
    parser.add_argument("--tune-name", dest="tune_name", default=None)
    parser.add_argument(
        "--mlflow-pattern",
    ...
    parser.add_argument("--seesaw-epoch-start", type=int, default=None)
    parser.add_argument("--seesaw-batch-multiplier", type=float, default=None)

    parser.add_argument("--aboba-strategy", default=None)
    parser.add_argument("--aboba-batch-multipliers", type=float, nargs="+", default=[1.0])
    parser.add_argument("--aboba-epoch-start", type=int, default=None)
...
def _load_reference_config(path: Path) -> Dict[str, Any]:
    with path.open() as f:
        return json.load(f)


def _base_config(reference: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    base = dict(reference)
    overrides = {
        ...
        "adaptive_batch_min": args.adaptive_batch_min,
        "adaptive_batch_max": args.adaptive_batch_max,
        "adaptive_batch_beta": args.adaptive_batch_beta,
        "report_to": args.report_to,
        "use_old_tune_params": args.use_old_tune_params,
        "tune_runs": args.tune_runs,
        "n_epoches_tune": args.n_epoches_tune,
    ...
    optimizer_name = str(base.get("optimizer", "sgd")).lower()
    if optimizer_name == "sgd":
        if args.momentum is not None:
            base["momentum"] = args.momentum
    else:
        base.pop("momentum", None)
    return base
...
def _format_name(
    ...
    name = pattern.format(
        optimizer=base.get("optimizer", "sgd"),
        dataset=base.get("dataset", "cifar10"),
        variant=variant,
...
def _apply_variant(
    ...
    if variant == "basic":
        ...
        cfg["epoch_start_ab"] = int(cfg.get("epoch_start_ab", 1))
        cfg.pop("adaptive_batch_strategy", None)
        cfg.pop("seesaw_alpha", None)
    elif variant == "seesaw":
        cfg["adaptive_batch"] = True
        cfg["adaptive_batch_strategy"] = "seesaw"
        cfg["seesaw_alpha"] = args.seesaw_alpha
        cfg["batch_size_multiplier"] = (
            args.seesaw_batch_multiplier
            if args.seesaw_batch_multiplier is not None
            else float(cfg.get("batch_size_multiplier", 1.0))
        )
        if args.seesaw_epoch_start is not None:
            cfg["epoch_start_ab"] = args.seesaw_epoch_start
    elif variant == "aboba":
        cfg["adaptive_batch"] = True
        cfg["adaptive_batch_strategy"] = args.aboba_strategy or str(
            cfg.get("adaptive_batch_strategy", "variance_ratio_iter")
        )
        cfg["batch_size_multiplier"] = aboba_batch_multiplier
        cfg.pop("seesaw_alpha", None)
        if args.aboba_epoch_start is not None:
            cfg["epoch_start_ab"] = args.aboba_epoch_start
    else:
...
def _write_config(path: Path, cfg: Dict[str, Any], overwrite: bool) -> None:
...
def main() -> None:
    ...
    reference = _load_reference_config(args.reference_config)
    base = _base_config(reference, args)
    ...
    for seed, bs in _pairs(args.seeds, args.basic_batch_sizes):
        ...
        path = output_root / "basic" / f"bs{bs}-seed{seed}.json"
    ...
    for seed, bs in _pairs(args.seeds, args.seesaw_batch_sizes):
        ...
        path = output_root / "seesaw" / f"bs{bs}-seed{seed}.json"
    ...
    for mult in args.aboba_batch_multipliers:
        for seed, bs in _pairs(args.seeds, args.aboba_batch_sizes):
            ...
            path = output_root / "aboba" / f"abm{suffix}" / f"bs{bs}-seed{seed}.json"
            plan.append(("aboba", path, cfg))

    run_order = sorted(plan, key=lambda item: item[1].as_posix())
    run_commands = [_run_command(output_root, path) for _, path, _ in run_order]
    planned_paths = [path for _, path, _ in plan]
    stale_configs = _find_stale_json_configs(output_root, planned_paths)
    ...
    if args.write_run_scripts:
        for part_idx in range(args.run_split_parts):
            cmds = [
                ...
                if cmd_idx % args.run_split_parts == part_idx
            ]
    ...
    if args.write_run_scripts:
        _write_run_script(output_root / "run-all.sh", run_commands)
        for idx, cmds in enumerate(part_commands, start=1):
    ...
    print(f"Wrote {len(plan)} configs -> {output_root} ({summary})")
    if args.prune_stale_configs:
