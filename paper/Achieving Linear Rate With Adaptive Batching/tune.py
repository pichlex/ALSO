...
def _tuned_params_path(dataset: str, name: str) -> str:
    base_dir = os.path.join(os.path.dirname(__file__), "tuned_params", dataset)
    os.makedirs(base_dir, exist_ok=True)
    return os.path.join(base_dir, f"{name}.json")


def load_tuned_params(dataset: str, name: str) -> Optional[Dict[str, Any]]:
    path = _tuned_params_path(dataset, name)
...
def save_tuned_params(dataset: str, name: str, params: Dict[str, Any]) -> None:
    path = _tuned_params_path(dataset, name)
...
def run_tuning(base_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    existing = load_tuned_params(dataset, tune_name) if use_old else None
    ...
    save_tuned_params(dataset, tune_name, best_params)
    cfg = base_config.copy()
